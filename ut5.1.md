...
- **REST** : La más común. Utiliza HTTP (GET, POST, PUT, DELETE) y suele devolver JSON. Es ligera y fácil de implementar.
- **SOAP**: Estándar veterano y más pesado debido al XML, pero ofrece una seguridad y transaccionalidad muy robustas (estándares WS-Security).
- **gRPC**: Desarrollado por Google. Utiliza HTTP/2 para transmitir datos binarios, lo que lo hace increíblemente rápido comparado con el texto plano.
- **GraphQL**: Creado por Facebook. Evita el "over-fetching" (traer datos de más) permitiendo que el desarrollador defina la estructura exacta de la respuesta.
...
| DELETE      | Para eliminar una entrada concreta en el servidor.                                                    |
...
El uso correcto de variables permite reutilizar colecciones sin modificar URLs ni scripts.
...
1. Hacer click en la columna central para seleccionar o crear un endpoint.
2. Ya hay un método *GET* creado (por defecto) y se le puede agregar una ruta.
3. Debajo de la ruta agregar la descripción para la documentación.
4. En las pestañas *status&body* para el *código 200* (por defecto) se puede agregar el **mensaje devuelto** (por ejemplo, en formato **JSON**)
5. También se pueden agregar campos devueltos en la pestaña *headers*.
6. Se pueden crear nuevos endpoints a la API con otros métodos (pj simular un PUT)
...
```
> *HttpURLConnection* es funcional pero está en desuso en proyectos modernos.
